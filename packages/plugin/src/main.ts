        ...
        HasSettings<ObsidianLiveSyncSettings>
{
    ...
    override onload() {
        ...
        // Managed Sync: handle OAuth callback via obsidian://oms-auth?token=...
        this.registerObsidianProtocolHandler("oms-auth", async (params) => {
            const { parseCallbackToken, decodeTokenEmail, getTokenExpiry } = await import("./auth/oauth");
            const { fetchCredentials, applyCredentialsToSettings } = await import("./auth/credentials");
            const { MANAGED_BACKEND_URL } = await import("./auth/types");

            const token = parseCallbackToken(new URLSearchParams(params as any));
            if (!token) return;

            const email = decodeTokenEmail(token);
            const expiresAt = getTokenExpiry(token);

            (this.settings as any)._managedAuth = { token, email, expiresAt };

            try {
                const creds = await fetchCredentials(MANAGED_BACKEND_URL, token);
                applyCredentialsToSettings(creds, this.settings);
                await this.saveSettings();
                new Notice(`Connected as ${email}. Vault sync configured automatically.`);
            } catch (e: any) {
                new Notice(`Failed to fetch credentials: ${e.message}`);
            }
        });
    }
    override onunload() {
