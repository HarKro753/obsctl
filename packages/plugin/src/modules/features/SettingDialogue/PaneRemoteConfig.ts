...
import { renderAuthScreen } from "../../../ui/AuthScreen.ts";
import { MANAGED_BACKEND_URL } from "../../../auth/types.ts";
import { isTokenExpired } from "../../../auth/credentials.ts";
...
export function paneRemoteConfig(
    ...
    // --- Managed Sync auth screen ---
    const storedAuth = (this.plugin.settings as any)._managedAuth ?? null;
    const isManagedSignedIn = storedAuth !== null && !isTokenExpired(storedAuth);

    const authContainer = paneEl.createDiv();
    renderAuthScreen({
        containerEl: authContainer,
        plugin: this.plugin,
    ...
    // Wrapper for CouchDB manual config — hidden when signed in via managed sync
    const manualCouchDBEl = paneEl.createDiv({ cls: "oms-manual-couchdb" });
    if (isManagedSignedIn) {
        manualCouchDBEl.style.display = "none";
    }
    ...
    {
        void addPanel(manualCouchDBEl, $msg("obsidianLiveSyncSettingTab.titleRemoteServer"), () => {}).then((paneEl) => {
    ...
    }
    {
        ...
        void addPanel(manualCouchDBEl, $msg("obsidianLiveSyncSettingTab.titleCouchDB"), () => {}).then((paneEl) => {
